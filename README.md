...
当前版本：V4.4.1 Default Preference Quick Start
...
- V4.3.3：增强主地点聚合。在 V4.3.2 基础上新增景区内部子景点聚合（翠湖·阮堤 / 翠湖公园游乐场 → 翠湖公园）、高校内部学院/校区聚合（云南大学呈贡校区新闻学院 → 云南大学）、住宿类 POI 过滤（民宿/酒店/客栈不作为景点推荐）。重点解决昆明测试中「翠湖被拆成很多内部景点」和「云南大学被拆成很多学院/校区」的问题。前后端去重逻辑同步增强，真实高德 POI / 后端 Mock / 前端 Mock 三条链路均应用。
- V4.3.4：全国通用 POI 主地点聚合。高校名称不再依赖城市白名单，含「大学」的子单位会优先聚合到第一个非「大学城」的大学主名；景区、公园、古镇、街区、博物馆、寺庙、湖山岛等内部子地点按分隔符和主地点词聚合；停车场、入口、售票处、游客中心、厕所、交通站点、住宿、餐饮、便利店、写字楼、小区等低价值辅助 POI 默认过滤。前后端规则继续保持一致。
- V4.3.5：修复 V4.3.4 去重过滤过度。高德 POI 入口改为先归并主地点、再过滤无法归并的低价值 POI；高校、公园、景区、博物馆等可识别主地点增加保护，避免「云南大学东门 / 云南大学图书馆 / 云南大学呈贡校区生命科学学院」在归并成「云南大学」之前被直接删除。
- V4.4：本地用户偏好保存。前端新增 `travel_ai_planner_user_preferences_v1` localStorage 偏好档案，可保存常用行程强度、起床偏好、预算、交通、住宿偏好、兴趣和避雷标签；新建计划时可一键复用。目的地、日期、人数不会作为长期偏好保存。数据仅保存在当前浏览器，不会上传服务器；换设备不会同步，清理浏览器缓存后会丢失。
- V4.4.1：默认偏好快速开始。新增快速开始表单（QuickStartForm.vue），使用默认偏好时只需补充目的地、日期、人数和住宿位置，无需重复回答已保存的长期偏好问题。完整问答流程仍然保留，用户可选择"重新填写完整问答"。
